...
def create_app():
    ...
    def user_profile():
        """This is for getting all the tracks for each of the user's playlists. NOTE: still a work in progress."""
        playlist_ids=[]
        sp = get_sp(session)
        ...
        aa = sp.user_playlist_tracks(user=user_id, playlist_id=playlist_ids[1])["items"]
        return str(aa)
